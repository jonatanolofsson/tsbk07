...
// Note: Some calls changed to have the result LAST
// This should be the standard over the whole unit.

// Note! Expanded version, several calls added compred to lab 2.
...
// Alla matriser är arrays av GLfloat!
	...
	void Mult(/*GLfloat *dest,*/ GLfloat *a, GLfloat *b, GLfloat *dest) // m = a * b
	{
		...
		for (x = 0; x <= 3; x++)
			for (y = 0; y <= 3; y++)
//				m[x*4 + y] = a[y][0] * b[0][x] + a[y][1] * b[1][x] + a[y][2] * b[2][x] + a[y][3] * b[3][x];
